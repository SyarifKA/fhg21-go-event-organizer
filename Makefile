...
port ?= 5432
user ?= postgres
pass ?= b7ea2db382224b5aafaa17dcacfe09e2
db ?= event_organizer

migrate\:init:
	PGPASSWORD=$(pass) psql -U$(user) -d postgres -p $(port) -c "create database $(db);"

migrate\:drop:
	PGPASSWORD=$(pass) psql -U$(user) -d postgres -p $(port) -c "drop database if exists $(db) with (force);"
